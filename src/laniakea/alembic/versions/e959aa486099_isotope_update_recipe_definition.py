...
def upgrade():
    ...
    op.add_column(
        'image_build_recipes', sa.Column('format', sa.Enum('UNKNOWN', 'ISO', 'IMG', name='imageformat'), nullable=True)
    )
    ...
    op.alter_column('image_build_recipes', 'distribution', existing_type=sa.TEXT(), nullable=False)
    op.alter_column('image_build_recipes', 'git_url', existing_type=sa.TEXT(), nullable=False)
    op.alter_column('image_build_recipes', 'suite', existing_type=sa.TEXT(), nullable=False)
...
def downgrade():
    ...
    op.add_column(
        'image_build_recipes',
        sa.Column(
            'kind', postgresql.ENUM('UNKNOWN', 'ISO', 'IMG', name='imagekind'), autoincrement=False, nullable=True
        ),
    )
    op.alter_column('image_build_recipes', 'suite', existing_type=sa.TEXT(), nullable=True)
    op.alter_column('image_build_recipes', 'git_url', existing_type=sa.TEXT(), nullable=True)
    op.alter_column('image_build_recipes', 'distribution', existing_type=sa.TEXT(), nullable=True)
