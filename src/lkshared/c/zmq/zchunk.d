...
//  Return chunk data
ubyte* zchunk_data (zchunk_t* self);
...
size_t zchunk_fill (zchunk_t* self, ubyte filler, size_t size);
...
//  Caller owns return value and must destroy it when done.
zchunk_t* zchunk_slurp (const(char)* filename, size_t maxsize);
...
//  Caller owns return value and must destroy it when done.
char* zchunk_strhex (zchunk_t* self);

//  Return chunk data copied into freshly allocated string
//  Caller must free string when finished with it.        
//  Caller owns return value and must destroy it when done.
char* zchunk_strdup (zchunk_t* self);

//  Return TRUE if chunk body is equal to string, excluding terminator
bool zchunk_streq (zchunk_t* self, const(char)* string);

//  Transform zchunk into a zframe that can be sent in a message.
//  Caller owns return value and must destroy it when done.
zframe_t* zchunk_pack (zchunk_t* self);

//  Transform a zframe into a zchunk.
//  Caller owns return value and must destroy it when done.
zchunk_t* zchunk_unpack (zframe_t* frame);

//  Calculate SHA1 digest for chunk, using zdigest class.
const(char)* zchunk_digest (zchunk_t* self);

//  Dump chunk to FILE stream, for debugging and tracing.
void zchunk_fprint (zchunk_t* self, FILE* file);
...
bool zchunk_is (void* self);

//  Self test of this class.
void zchunk_test (bool verbose);
